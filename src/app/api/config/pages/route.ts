...
// Build-time discovered page keys — generated by scripts/discover-pages.js
// before `next build`. Bundled into the production output so the admin
// auto-discovers newly created pages even on Vercel (where src/app/ is
// not available at runtime).
//
// Next.js supports JSON imports natively (resolveJsonModule=true in tsconfig).
// The file is regenerated on every `npm run build`, so it always reflects
// the current set of page directories under src/app/.
import discoveredPagesRaw from '@/lib/discovered-pages.json';

const DISCOVERED_PAGES: string[] = Array.isArray(discoveredPagesRaw)
  ? discoveredPagesRaw.filter((k): k is string => typeof k === 'string')
  : [];
 ...
 *   1. KNOWN_PAGES — built-in pages with friendly labels (always present)
 *   2. discovered-pages.json — pages found by build-time scan of
 *      src/app/<dir>/page.tsx. Works in production (Vercel) because
 *      the JSON is bundled into the build output.
 *   3. AdPlacement.page values from the DB — ads assigned to a custom
 *      page key (e.g. "blog") make that key automatically tab-eligible
 *   4. Runtime filesystem scan of src/app/<dir>/page.tsx — only works
 *      in development; silently no-ops in production.
 *
...
export async function GET() {
  ...
  try {
    ...
    // 2. Build-time discovered pages (from discovered-pages.json bundled at build)
    for (const key of DISCOVERED_PAGES) {
      if (key === GLOBAL_PAGE_KEY) continue;
      if (pages.some(p => p.key === key)) continue;
      pages.push({ key, label: pageLabel(key) });
    }

    // 3. Distinct page values from AdPlacement table
    ...
    // 4. Runtime filesystem scan — only works in development where
    // src/app/ exists at process.cwd(). In production (Vercel standalone),
    // this silently no-ops; the build-time JSON above covers production.
    try {
      ...
      // build-time discovered-pages.json + DB-distinct covers production.
    }
